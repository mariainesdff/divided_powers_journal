...
@[simp]
theorem mem_dite_empty_left {α : Type*} (p : Prop) [inst : Decidable p]
    (t : ¬p → Finset α) (x : α) :
    (x ∈ if h : p then ∅ else t h) ↔ ∃ (h : ¬p), x ∈ t h := by
  simp only [mem_dite, notMem_empty, imp_false]
  exact ⟨fun h => ⟨h.1, h.2 h.1⟩, fun ⟨h₁, h₂⟩ => ⟨h₁, fun _ => h₂⟩⟩

@[simp]
theorem mem_ite_empty_left (p : Prop) [Decidable p] (t : Finset α) (x : α) :
    x ∈ ite p ∅ t ↔ ¬p ∧ x ∈ t :=
  (Finset.mem_dite_empty_left p (fun _ => t) x).trans (by simp)

@[simp]
theorem mem_dite_empty_right {α : Type*} (p : Prop) [inst : Decidable p]
    (t : p → Finset α) (x : α) :
    (x ∈ if h : p then t h else ∅) ↔ ∃ (h : p), x ∈ t h := by
  simp only [mem_dite, notMem_empty, imp_false, Decidable.not_not]
  exact ⟨fun h => ⟨h.2, h.1 h.2⟩, fun ⟨h₁, h₂⟩ => ⟨fun _ => h₂, h₁⟩⟩

@[simp]
theorem mem_ite_empty_right (p : Prop) [Decidable p] (t : Finset α) (x : α) :
    x ∈ ite p t ∅ ↔ p ∧ x ∈ t :=
  (Finset.mem_dite_empty_right p (fun _ => t) x).trans (by simp)

end Finset

namespace MvPowerSeries

theorem support_monomial {σ : Type*} {n : σ →₀ ℕ} {a : R} [h : Decidable (a = 0)] :
    (MvPowerSeries.monomial R n a).support = if a = 0 then ∅ else {n} := by
  rw [← MvPolynomial.coe_monomial]
  have : (((MvPolynomial.monomial n) a).support : Set (σ →₀ ℕ)) = if a = 0 then ∅ else {n} := by
    rw [MvPolynomial.support_monomial]
    ext d
    simp [mem_coe, mem_ite_empty_left, mem_singleton, Set.mem_ite_empty_left,
      Set.mem_singleton_iff]
  rw [← this]
  ext d
  simp only [Function.mem_support, ne_eq, mem_coe, MvPolynomial.mem_support_iff]
  rw [← MvPowerSeries.coeff_apply R ↑((MvPolynomial.monomial n) a) d, MvPolynomial.coeff_coe]

theorem support_X {σ : Type*} {n : σ} [Nontrivial R] :
    Function.support (MvPowerSeries.X (R := R) n) = {Finsupp.single n 1} := by
  classical
  rw [MvPowerSeries.X_def]
  ext d
  rw [MvPowerSeries.support_monomial, if_neg one_ne_zero]

theorem support_add {σ : Type*} {p q : MvPowerSeries σ R} :
    Function.support (p + q) ⊆ Function.support p ∪ Function.support q := Function.support_add _ _

theorem degree_eq_one_of_mem_support_X {σ : Type*} [Nontrivial R] {s : σ} {d : σ →₀ ℕ}
    (hd : d ∈ Function.support (MvPowerSeries.X (σ := σ) (R := R) s)) :
    d.degree = 1 := by
  rw [MvPowerSeries.support_X] at hd
  simp only [Fin.isValue, Set.mem_singleton_iff] at hd
  rw [hd]
  exact degree_single _ _
...
lemma coeff_subst_single {σ : Type*} [DecidableEq σ] (s : σ) (f : R⟦X⟧) (e : σ →₀ ℕ) :
  ...
  rw [coeff_subst (hasSubst_iff.mpr (MvPowerSeries.hasSubst_of_constantCoeff_zero (by simp))),
...
protected theorem mul {f g : PowerSeries R} (hf : IsExponential f) (hg : IsExponential g) :
  ...
  add_mul := by
    repeat rw [← coe_substAlgHom (hasSubst_iff.mpr
      (MvPowerSeries.hasSubst_of_constantCoeff_zero (by simp)))]
...
protected theorem rescale (a : A) {f : PowerSeries R} (hf : IsExponential f) :
  ...
  add_mul := by
    ...
    · repeat rw [subst_linear_subst_scalar_comm]
      ...
      repeat
        intro d hd
        exact MvPowerSeries.degree_eq_one_of_mem_support_X hd
      -- The first application of `subst_linear_subst_scalar_comm` is a bit different.
      · intro d hd
        rcases MvPowerSeries.support_add hd with (hd0 | hd1)
        · exact MvPowerSeries.degree_eq_one_of_mem_support_X hd0
        · exact MvPowerSeries.degree_eq_one_of_mem_support_X hd1
    · simp only [nontrivial_iff, ne_eq, not_exists, not_not] at h
      ext d
      apply h _ _
...
protected theorem rescale_add (r s : A) {f : R⟦X⟧} (hf : IsExponential f) :
  ...
  simp [Fin.isValue, map_add, MvPolynomial.aeval_X, add_smul, a, algebraMap_smul, Fin.isValue]
...
noncomputable instance : DistribMulAction A (Additive R⟦X⟧) where
  one_smul := by
    simp only [Additive.forall, toAdditive_smul_coe, map_one, rescale_one, RingHom.id_apply,
      implies_true]
  mul_smul  := by
    simp only [Additive.forall, toAdditive_smul_coe, map_mul]
    intro _ _ _
    rw [mul_comm, ← rescale_rescale]
  smul_zero a := by
...
instance instIsScalarTower (R : Type*) [CommRing R] (S : Type*) [CommRing S] [Algebra R S]
  ...
  smul_assoc r s f := by
    apply coe_injective
    simp only [coe_smul]
    rw [← algebraMap_smul S, smul_eq_mul, map_mul, mul_comm, ← rescale_rescale]
    apply congr_fun
    ext f n
